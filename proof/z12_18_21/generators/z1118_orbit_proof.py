...
def patterns():return tuple(p for p in partitions(18,102,11) if sum(C(x,3) for x in p)<=330)
...
def generate(idx,out,limit):
    ps=patterns();assert len(ps)==51;p=ps[idx];rem=dict(Counter(p));k=max(rem);first=(1<<k)-1;rem[k]-=1
    t=time.time();g=Gen(limit);tree=g.outer([first],rem,{})
    obj={'format':'Z1118-local-v1','index':idx,'pattern':p,'first':first,'tree':tree,'stats':{'outer':g.o,'inner':g.i,'duals':g.d,'branches':g.br,'maxdepth':g.md,'seconds':time.time()-t}}
