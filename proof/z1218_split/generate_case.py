...
def profiles101():
    ...
    def rec(pos,last,rem,v):
        if pos==18:
            if rem==0 and sum(math.comb(d,3) for d in v)<=330: out.append(tuple(v))
            return
        left=18-pos
...
def row8_cases():
    out=[]
    for pi,p in enumerate(profiles101()):
        slack=330-sum(math.comb(d,3) for d in p); cnt=Counter(p); vals=sorted(cnt); sels=set()
        def rec(i,left,cur):
            if i==len(vals):
                if left==0:
                    s=[]
...
def build_no8():
    ...
    add_row_lex(cnf); add_k33(cnf)
    return cnf,{'branch':'no8'}

def build_row8(caseid):
    pi,slack,sel,unsel=row8_cases()[caseid]; cnf=CNF()
...
def main():
    ...
    cnf,meta=build_no8() if a.branch=='no8' else build_row8(a.case); write(cnf,a.out,meta); print(meta); print('vars',cnf.nvars,'clauses',len(cnf.clauses))
