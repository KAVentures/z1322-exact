...
def main():
    ...
    # The base CNF lex-sorts columns inside each marked-row incidence class.
    # Therefore row 1 is a prefix of ones then zeros in each class.
    for c in range(marked):
        cnf.unit(x(1,c) if c < args.a else -x(1,c))
    for c in range(marked,C):
        cnf.unit(x(1,c) if c-marked < args.b else -x(1,c))
