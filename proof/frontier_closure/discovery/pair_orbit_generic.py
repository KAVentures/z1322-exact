...
def cells_of(fixed):
 g=defaultdict(list)
 for r in range(V):g[sum(((b>>r)&1)<<j for j,b in enumerate(fixed))].append(r)
...
def build(fixed,remaining,forbidden):
 cells=cells_of(fixed);sizes=tuple(map(len,cells));pos=[]
 ...
 for k,need in sorted(remaining.items(),reverse=True):
  ...
  for p in profiles(sizes,k):
   ...
   o=osize(sizes,p);up=min(need,2*o-(fc[r] if o==1 else 0))
   if up<=0:continue
   h=tuple(prod(C(x,y) for x,y in zip(p,u)) for u,_,_ in pos)
   if any(fm==2 and q for q,(_u,fm,_o) in zip(h,pos)):continue
   vars.append((k,p,r,o,up,h))
 return cells,tuple(pos),tuple(vars),False

def model(remaining,pos,vars):
 A=[];b=[]
 for qi,(_u,fm,o) in enumerate(pos):A.append(tuple(v[5][qi] for v in vars));b.append((2-fm)*o)
...
class Gen:
 ...
 def outer(self,fixed,remaining,forbidden,depth=0):
  ...
  cells,pos,vars,bad=build(fixed,remaining,forbidden)
  if bad:return ['G']
  A,b=model(remaining,pos,vars);k=max(q for q,n in remaining.items() if n>0);cs=sorted((v for v in vars if v[0]==k),key=lambda v:v[1])
  if not cs:return ['N',k]
  lo=[0]*len(vars);up=[v[4] for v in vars];rr=self.lp(A,b,lo,up);p=None
  if rr is not None and rr.success:
   ...
   if dq:
    ...
    if not (mr.x is not None and -mr.fun>=need-1e-7):p=self.inner(A,b,vars,need,lo,up)
  else:p=self.inner(A,b,vars,need,lo,up)
  ...
  for v in cs:
   ...
   z=self.outer(fixed+[r],nr,nf,depth+1)
   if z is None:return None
   ch.append(z);prior.update(orbit_masks(cells,pr))
  return ['B',k,ch]
class Verify:
 ...
 def outer(self,node,fixed,remaining,forbidden):
  self.o+=1;need=sum(max(0,n) for n in remaining.values());cells,pos,vars,bad=build(fixed,remaining,forbidden)
  if node[0]=='G':assert bad;return
  assert not bad;A,b=model(remaining,pos,vars);k=max(q for q,n in remaining.items() if n>0);cs=sorted((v for v in vars if v[0]==k),key=lambda v:v[1])
...
def generate(out,limit,counts):
 counts={int(k):int(v) for k,v in counts.items() if int(v)>0};k=max(counts);fixed=[(1<<k)-1];rem=dict(counts);rem[k]-=1;g=Gen(limit);t=time.time();tree=g.outer(fixed,rem,{})
 if tree is None:raise RuntimeError('aggregate feasible branch found')
 obj={'format':'pair-orbit-generic-v1','counts':counts,'fixed':fixed,'remaining':rem,'tree':tree,'stats':{'outer':g.o,'inner':g.i,'duals':g.d,'branches':g.br,'maxdepth':g.md,'seconds':time.time()-t}}
 with gzip.open(out,'wt',encoding='utf8',compresslevel=9) as f:json.dump(obj,f,separators=(',',':'))
 v=Verify();v.outer(tree,fixed,rem,{});print('DONE',obj['stats'],'VERIFY',v.__dict__,'bytes',os.path.getsize(out))
def verify(path):
 with gzip.open(path,'rt',encoding='utf8') as f:o=json.load(f)
 assert o['format'] in ('pair-orbit-a4-v1','pair-orbit-generic-v1');v=Verify();v.outer(o['tree'],o['fixed'],{int(k):v for k,v in o['remaining'].items()},{});print('VERIFIED',v.__dict__)
if __name__=='__main__':
 ap=argparse.ArgumentParser();sp=ap.add_subparsers(dest='cmd',required=True);g=sp.add_parser('generate');g.add_argument('out');g.add_argument('--limit',type=int,default=1000000);g.add_argument('--counts',required=True,help='JSON object, e.g. {"5":4,"6":5}');v=sp.add_parser('verify');v.add_argument('path');a=ap.parse_args();generate(a.out,a.limit,json.loads(a.counts)) if a.cmd=='generate' else verify(a.path)
