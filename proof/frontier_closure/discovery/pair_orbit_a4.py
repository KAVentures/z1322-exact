...
def generate(out,limit):
 fixed=[(1<<6)-1];rem={6:4,5:4};g=Gen(limit);t=time.time();tree=g.outer(fixed,rem,{})
 if tree is None:raise RuntimeError('aggregate feasible branch found')
 obj={'format':'pair-orbit-a4-v1','fixed':fixed,'remaining':rem,'tree':tree,'stats':{'outer':g.o,'inner':g.i,'duals':g.d,'branches':g.br,'maxdepth':g.md,'seconds':time.time()-t}}
...
def verify(path):
 ...
 assert o['format']=='pair-orbit-a4-v1';v=Verify();v.outer(o['tree'],o['fixed'],{int(k):v for k,v in o['remaining'].items()},{});print('VERIFIED',v.__dict__)
if __name__=='__main__':
 ap=argparse.ArgumentParser();sp=ap.add_subparsers(dest='cmd',required=True);g=sp.add_parser('generate');g.add_argument('out');g.add_argument('--limit',type=int,default=1000000);v=sp.add_parser('verify');v.add_argument('path');a=ap.parse_args();generate(a.out,a.limit) if a.cmd=='generate' else verify(a.path)
